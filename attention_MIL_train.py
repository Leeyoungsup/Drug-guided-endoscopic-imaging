...
test_data=pd.read_csv('../../data/mteg_data/external/external1.csv') 
file_path='../../data/mteg_data/external/external1/'
test_image_list=[]
for i in range(len(test_data)):
    file_name=test_data.loc[i]['File Name']
    id=file_name[:file_name.find('_')]
    test_image_list.append(file_path+id)
label_data=pd.read_csv('../../data/mteg_data/external/label.csv')  
test_label_list=[]
test_id_list=[]
...
for epoch in range(300):
    ...
    with torch.no_grad():
        ...
        torch.save(model.state_dict(), '../../model/mteg/external/external1/f1_'+str(round(f1,4))+'.pt')
end = time.time()
d = datetime.datetime.now()
now_time = f"{d.year}-{d.month}-{d.day} {d.hour}:{d.minute}:{d.second}"
